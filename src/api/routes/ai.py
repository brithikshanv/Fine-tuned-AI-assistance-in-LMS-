...
async def ai_response_for_question(request: AIChatRequest):
    ...
    if request.task_type == TaskType.QUIZ:
        ...
        if language == "Tamil":
            lang_prompt = '''Give the response in a way a tamil medium student can understand using a mix of Tamil & English words. That is, the response should be in Tanglish

                            Eg., 
                            English : React is a JavaScript framework. React is used to develop web applications easily.
                            Tanglish : React என்பது  JavaScript framework. Easy-ஆ web application develop பண்ண React-அ use பண்ணலாம்.'''
        else:
            lang_prompt = "Give  response in English."
